...
//
// (되돌린 시도) "raw가 정지거리mm보다 가까우면 기준값과 무관하게 항상 장애물로
// 본다"는 안전장치를 한 번 넣어봤는데, 실측에서 심각한 회귀를 일으켜서 되돌렸다.
// row7(바닥) 같은 칸은 회전을 해도 거리가 거의 안 바뀌는데(항상 ~200mm대,
// 정지거리mm=250보다 가까움), 그 안전장치가 기준값 비교를 건너뛰고 무조건
// 장애물로 만들어버려서 어느 방향을 봐도 정면안전()이 절대 true가 안 되고
// 회전 탐색이 매번 소진→재시도만 무한 반복했다(로그로 확인됨). 카운트다운을
// 마침 벽 앞에서 누르면 그 벽이 평생 "열림"으로 보이는 한계는 남아있지만,
// (기존 AUTONOMOUS_WANDER_EXAMPLE.md도 같은 한계가 있었음) 트인 곳에서
// 보정하라는 사용 안내로 대응하는 게, 바닥을 영원히 못 빠져나가는 것보다 낫다.
