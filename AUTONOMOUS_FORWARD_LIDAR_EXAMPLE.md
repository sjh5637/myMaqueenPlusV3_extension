...
// (예: 700↔4000↔300 식). 백그라운드 스캐너 한 바퀴(약 0.7~0.8초)가 메인 루프
// 틱(40ms)보다 훨씬 느려서, delta 비교가 사실상 "거의 1초 전 값과 지금 값"을
// 비교하는 셈인데, 이 시간 간격에서 바닥 쪽 행은 노이즈만으로도 80mm 이상
// 차이가 자주 나서 진짜 장애물 없이도 긴급정지가 계속 발동했다. row0/1로
// 좁혀서 노이즈가 적은 행만 긴급 판단에 쓴다 — 이 좁히기가 맞는지는 아래
// 전체64로그()/델타64로그() 출력으로 직접 확인해 필요하면 row 범위를 조정한다.
