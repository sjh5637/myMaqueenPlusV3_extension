...
라이다는 `matrixLidarDistance.matrixPointOutput()`으로 8x8 점을 직접 읽되,
백그라운드(`control.inBackground()`)에서 계속 64칸 캐시를 갱신해 메인 루프는
즉시 캐시만 읍습니다. 시작 시점에 캡처한 배경(바닥 등) 기준값과 비교해 칸별로
"열림/장애물/모름"을 판정하고, 직전 틱과의 변화량(delta)으로 급격한 근접(긴급
후진)과 헛돌이/안 보이는 장애물에 막힘을 실시간으로 감지합니다. 평상시엔 중앙
4열만 보는 저비용 체크로 정지 여부를 판단하고, 막히면 `pidControlAngle(..,
Allowed)`로 회전을 시작해 매 틱 캐시를 보다가 충분한 공간이 보이는 순간
회전 중간에라도 즉시 멈춰 전진으로 전환합니다(랜덤 좌/우 시작). 이 회전 탐색이
반복 실패하면 기존 360도 굵게→세밀 탐색으로 최종 폴백합니다.
...
2. `B`를 누르면 3초 카운트다운(끝에 배경 기준값 캡처) 후 평상시 회피 주행이
   시작됩니다.
3. 정면 중앙 4열이 트여 있으면 그대로 전진합니다. 막히면 멈추고 랜덤 좌/우
   방향으로 회전 탐색을 시작해, 충분한 공간을 찾는 즉시(회전 중이라도) 멈추고
   전진으로 전환합니다.
4. 회전 탐색이 반복 실패하면 360도 굵게→세밀 탐색으로 빠져나갈 방향을 찾습니다.
   그래도 못 찾으면 정지하고 X 아이콘을 표시합니다.
5. 회전/전진/탐색 중 언제든 정면이 한 틱 사이 갑자기 많이 가까워지면(긴급
   delta) 즉시 후진합니다. 전진 중 진행이 없으면(헛돌이/안 보이는 장애물)
   다른 방향을 시도합니다.
...
// -1=모름(글리치), 양수=실제 장애물 거리. raw가 정지거리mm보다 가까우면 기준값과
// 무관하게 항상 장애물로 본다 — 카운트다운 끝나는 순간 로봇이 마침 벽/장애물을
// 마주보고 있었다면 그 거리가 그대로 "평소 배경"으로 저장되어, 그 장애물이 계속
// 있어도 기준과 똑같다는 이유로 영원히 "열림"으로 오판할 수 있었다(정지거리mm보다
// 훨씬 가까운데도 안 잡히는 구조적 구멍). 정지거리mm 안쪽은 이 안전 바닥으로 항상
// 잡는다. 기준(그 칸의 평소 배경 거리)이 없거나(보정 전, raw==0 글리치였던 칸)
// 기준 자체가 "확실한 감지없음"이었던 칸이면 지금 읍은 값을 그대로 신뢰한다.
// 기준이 실측 거리(대개 바닥)였던 칸은, 지금 값이 그 기준보다 기준값여유mm 이상
// 가까워졌을 때만 "새 장애물"로 인정한다.
function 셀판정(raw: number, 기준: number): number {
    if (raw >= 라이다무효값mm) return 0
    if (raw == 0) return -1
    if (raw < 정지거리mm) return raw
...
// 추가로 정면블록확정()과 같은 식으로 "긴급확인필요(2)틱 연속" 확인을 거친다 —
// 단 한 틱짜리 튐(오작동)으로 즉시 발동하지 않고, 다음 틱에도 같은 위험 후보가
// 다시 보여야 진짜 위험으로 확정한다. 후보가 안 보이면 그 즉시 카운터를 리셋한다.
let 긴급연속 = 0
const 긴급확인필요 = 2
        ...
        for (let row = 0; row < 2; row++) {
            ...
            if (델타[idx] >= 긴급delta한계mm && 지금 > 0 && 지금 <= 긴급최대거리mm) {
                ...
                로그("DELTA EMERGENCY CANDIDATE " + 긴급연속 + "/" + 긴급확인필요
                    + " col" + col + " row" + row + " delta" + 델타[idx] + " now" + 지금)
                return 긴급연속 >= 긴급확인필요
            }
        }
    }
...
basic.forever(function () {
    ...
    // 후진만 하고 끝내면 다음 틱에 다시 똑같이 막혀서 "긴급후진, 긴급후진"만
    // 반복할 수 있다 — 후진 직후 바로 회전 탐색을 시작해서, 후진으로 확보한
    // 공간에서 주변을 둘러보고 실제로 갈 수 있는 방향을 데이터로 찾게 한다.
    if (정면긴급위험()) {
        if (회전탐색중) {
            maqueenPlusV2.pidControlStop()
            회전탐색중 = false
...
| `적응전진거리cm` | 6 (4~10 가변) | 전진 거리, 성공/실패에 따라 가변 |
...
| `긴급delta한계mm` | 80 | 정면 칸이 한 틱 사이 이만큼 가까워지면 절대 거리와 무관하게 긴급 후진 |
