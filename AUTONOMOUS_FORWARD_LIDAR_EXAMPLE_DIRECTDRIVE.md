...
// 직접 속도 제어 관련 — controlMotor()에 줄 속도(0~255). 841 재시작/체감 과속을
// 줄이기 위해 시작값과 최저값을 45로 낮추고, 자동 가속 상한도 105로 제한한다.
const 기본전진속도 = 45
const 최소전진속도 = 45
const 최대전진속도 = 105
...
function 정면추적거리(): number {
    let 최소 = -1
    let 값 = 열값읽기범위(2, 정면판정행시작, 정면판정행끝, true)
    if (값 > 0 && (최소 < 0 || 값 < 최소)) 최소 = 값
    값 = 열값읽기범위(3, 정면판정행시작, 정면판정행끝, true)
    if (값 > 0 && (최소 < 0 || 값 < 최소)) 최소 = 값
    값 = 열값읽기범위(4, 정면판정행시작, 정면판정행끝, true)
    if (값 > 0 && (최소 < 0 || 값 < 최소)) 최소 = 값
    값 = 열값읽기범위(5, 정면판정행시작, 정면판정행끝, true)
    if (값 > 0 && (최소 < 0 || 값 < 최소)) 최소 = 값
    return 최소
}
...
function 정면막힘열수(추가여유mm: number, 행끝: number, 기준사용: boolean): number {
    let 막힌열 = 0
    if (!칸안전(열값읽기범위(2, 정면판정행시작, 행끝, 기준사용), 추가여유mm)) 막힌열 += 1
    if (!칸안전(열값읽기범위(3, 정면판정행시작, 행끝, 기준사용), 추가여유mm)) 막힌열 += 1
    if (!칸안전(열값읽기범위(4, 정면판정행시작, 행끝, 기준사용), 추가여유mm)) 막힌열 += 1
    if (!칸안전(열값읽기범위(5, 정면판정행시작, 행끝, 기준사용), 추가여유mm)) 막힌열 += 1
    return 막힌열
}
...
let 회전탐색반복횟수 = 0
const 회전1회각도 = 170
// 실측 주행 로그에서 ROTATE SEARCH START -> ROTATE FOUND CONFIRMED 간격이
// 거의 항상 9.8~10초로 일정하게 나왔다(편차 거의 없음). 기존 추정값(4000ms)이
// 실제보다 짧으면, 첫 170도 회전이 채 끝나기도 전에 회전탐색틱()이 타임아웃으로
// 오인해 같은 방향으로 170도 회전 명령을 또 발급하게 되고, 그 결과 거의 항상
// "추정값의 2배+확인 절차(약 1.8초)"에 가까운 시간이 걸린다 — 관찰된 9.8~10초가
// 정확히 4000*2+1.8s 근방과 맞아떨어진다. 실측값에 맞춰 늘려서 불필요한 재발급을
// 줄인다(회전 탐색이 전체 주행 시간 중 가장 큰 비중을 차지하는 구간이라, 전진
// 속도보다 이 부분을 줄이는 게 체감 속도 개선에 더 효과적이다).
const 회전1회예상ms = 8200
const 회전탐색최대반복 = 3
const 회전탐색확정추가각도 = 25
...
| `기본전진속도` | 45 | `controlMotor()`에 주는 시작 속도(0~255). 체감 과속과 841 재시작 가능성을 줄이기 위해 낮춤 |
| `최소전진속도` / `최대전진속도` | 45 / 105 | 정밀도 레벨에 따라 가변하는 속도 범위. 자동 가속 상한을 105로 제한 |
...
| `회전1회각도` / `회전1회예상ms` | 170 / 8200 | 회전 탐색 1회 호출 각도와 예상 소요시간(실측 로그 기반으로 조정 — 너무 짧으면 회전이 끝나기 전에 중복 재발급됨) |
...
기본값인 `디버그레벨=1`로 합니다. 841처럼 실행 중 재시작이 보이면 무선 로그/문자열 생성이
메모리를 압박하는지 먼저 의심해야 하므로, 이 버전은 하트비트 로그를 3초 간격의 짧은 형식으로 줄였습니다.
...
## 실전용(디버그/테스트 기능 제거) 버전

위 디버그용 코드와 회피/탈출 알고리즘은 완전히 동일합니다. 다음만 다릅니다.

- `로그()`/`상세로그()`가 빈 함수입니다 — 라디오 송신을 전혀 하지 않습니다.
  `radio.setGroup()`/`radio.setTransmitPower()` 호출도 없습니다(디버그 수신기가
  필요 없습니다).
- A 버튼 직진 raw 수집 테스트(`직진테스트*`)와 A+B 패킷 진단 모드
  (`라이다패킷진단로그`/`라이다고정점패킷문자열`)를 통째로 제거했습니다 — B
  버튼만 남아 자율주행을 시작합니다.
- 하트비트 로그, `SCAN CYCLE` 로그, `캐시최대나이ms()`처럼 로그 전용으로만
  쓰이던 코드를 제거했습니다.
- 그 외 라이다 캐시/기준값/delta/긴급 정지/장애물 조향/물리적 정지 감지/회전
  탐색/360도 탈출/LCD 표시는 위 코드와 한 글자도 다르지 않습니다 — 즉 튜닝값
  (`기본전진속도` 등)을 바꿀 때는 이 코드와 위 디버그 코드 양쪽에 똑같이
  반영해야 합니다.
