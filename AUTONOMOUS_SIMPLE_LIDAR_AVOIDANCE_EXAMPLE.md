...
스크린샷에 보이는 PID 블록은 "계속 돌리는 모터 블록"이 아니라, 마퀸플러스 V3 보드가 엔코더를 보고 정해진 거리나 각도만큼 움직이도록 맡기는 블록입니다.

| 블록 | 언제 쓰나 | 이 예제에서의 역할 |
|---|---|---|
| `PID 거리 제어` | 정해진 cm만큼 정확히 전진/후진할 때 | 이 간단 예제 본문에서는 쓰지 않습니다. 일정 거리 이동 미션을 추가할 때 사용합니다. |
| `PID 각도 제어` | 제자리에서 정해진 각도만큼 회전할 때 | 장애물을 만나 후진한 뒤 45도, 75도, 30도, 5도 단위로 방향을 바꿀 때 사용합니다. 코드에서는 `각도회전()` 안의 `maqueenPlusV2.pidControlAngle(...)`입니다. |
| `PID 제어 정지` | 실행 중인 PID 거리/각도 제어를 중간에 멈출 때 | 이 예제는 각도 회전이 끝날 때까지 기다리는 방식이라 직접 쓰지 않습니다. 다른 조건에서 회전을 강제로 끊고 싶을 때 사용합니다. |
| `실시간 속도 읽기` | 실제 바퀴가 돌고 있는지 확인할 때 | 전진 명령은 나갔는데 양쪽 바퀴 속도가 거의 0이면 벽이나 낮은 장애물에 걸린 것으로 보고 `STALL` 복구를 실행합니다. |

PID 블록의 `중단 허용/끝날 때까지 실행` 선택은 처음 보면 헷갈립니다.

- `중단 허용`: PID 동작을 시작한 뒤 다음 블록으로 바로 넘어갑니다. 뒤의 모터/PID 명령이 현재 동작을 끊을 수 있습니다.
- `끝날 때까지 실행`: 해당 PID 거리/각도 동작이 끝날 때까지 기다린 뒤 다음 블록으로 넘어갑니다.

이 예제의 회전은 정확도가 중요하므로 `PID 각도 제어`를 `끝날 때까지 실행`으로 호출합니다. 그래서 회전 중에는 다음 판단으로 넘어가지 않고, 회전이 끝난 다음 라이다를 다시 읽습니다.

## 블록으로 보면 어떻게 쓰이나

MakeCode 블록만으로 완전히 똑같이 만들기는 어렵지만, 구조는 아래 순서입니다.

1. `시작하면`: LCD 초기화, 상태 표시, 라이다 주소를 `Addr4(0x33)`로 정합니다.
2. `B 버튼을 누르면`: `주행중` 값을 켜고 끕니다. 켜질 때 속도, 실패 횟수, 마지막 회전 방향을 초기화합니다.
3. `무한 반복`: `주행중`이 아니면 LCD만 갱신하고 멈춰 있습니다.
4. `무한 반복`: 라이다의 핵심 열을 읽습니다. 이 예제는 8x8 전체를 항상 읽지 않고, 앞쪽 판단에 필요한 열을 중심으로 읽습니다.
5. 위험하면: 모터 정지 -> 짧게 후진 -> `PID 각도 제어`로 회전 -> 다시 라이다 읽기.
6. 위험하지 않으면: 좌/우 바퀴 속도를 조금 다르게 줘서 부드럽게 전진합니다.

즉, 이 예제의 핵심은 "라이다 읽기 -> 위험 판단 -> 직접 모터 전진 또는 PID 회전"입니다. PID 블록은 장애물을 보는 블록이 아니라, 이미 "돌아야 한다"고 판단한 뒤 실제 회전을 정확히 실행하는 블록입니다.

## 코드 읽는 순서

전체 코드를 처음부터 끝까지 한 줄씩 읽으면 어렵습니다. 아래 묶음만 먼저 보면 흐름이 잡힙니다.

| 코드 묶음 | 하는 일 | 먼저 볼 함수 |
|---|---|---|
| 시작/정지 | B 버튼으로 주행을 켜고 끕니다. 켤 때 실패 횟수와 마지막 회전 방향을 초기화합니다. | `input.onButtonPressed(Button.B, ...)` |
| 메인 판단 | 계속 반복하면서 라이다를 읽고, 위험/정지/막힘/전진 중 하나를 고릅니다. | `basic.forever(...)` |
| 라이다 읽기 | 8x8 중 필요한 열과 가장 아래 행을 거리값으로 바꿉니다. | `핵심장면읽기()`, `아래행위험읽기()` |
| 부드러운 전진 | 정면이 조금 가까워지면 좌우 바퀴 속도를 다르게 줘서 곡선으로 피합니다. | `자연회피전진()` |
| 회피 회전 | 너무 가까우면 멈추고 짧게 후진한 뒤 PID 각도 회전으로 방향을 바꿉니다. | `회피회전()`, `각도회전()` |
| 막혔을 때 복구 | 같은 방향 회피가 반복되거나 바퀴가 실제로 안 돌면 주변 각도를 훑어 더 나은 방향을 찾습니다. | `탐색복구()`, `부분탐색()`, `정밀탐색360세밀()` |

가장 중요한 판단은 `basic.forever(...)` 안의 `if / else if / else` 순서입니다. 위쪽 조건일수록 우선순위가 높습니다. `DANGER`는 즉시 회피, `STALL`은 전진 명령을 줬지만 바퀴가 안 도는 상황, `BLOCK`은 정면 중앙이 연속으로 가까운 상황, `RUN/CURVE/PASS`는 계속 전진 가능한 상황입니다.

## 라이다 좌표를 읽는 방식

`matrixPointOutput(주소, x, y)`는 8x8 라이다의 한 칸 거리를 mm 단위로 읽습니다.

- `x`: 왼쪽에서 오른쪽으로 0~7열입니다.
- `y`: 위에서 아래로 0~7행입니다.
- 반환값이 0 이하이거나 4000 이상이면 이 예제에서는 "유효한 장애물 없음"으로 처리합니다.
- 중앙 판단은 주로 `x=2~5`를 봅니다. 이 칸들이 가까우면 정면에 물체가 있다고 봅니다.
- `y=7`은 가장 아래 행입니다. 바닥 영향을 받을 수 있지만, 아주 가까운 충돌권을 빨리 잡는 보조 안전장치로만 씁니다.

예를 들어 정면 중앙이 가까운지 보려면 `x=2~5`에서 가장 작은 유효 거리를 찾습니다. 왼쪽과 오른쪽 중 어디가 더 넓은지 보려면 왼쪽 `x=0~2` 점수와 오른쪽 `x=5~7` 점수를 비교합니다.

## LCD 상태값 읽는 법

LCD는 디버그용입니다. 로봇이 왜 그 행동을 하는지 확인할 때 봅니다.

| 표시 | 뜻 |
|---|---|
| `STATE RUN` | 안전해서 전진 중 |
| `STATE CURVE` | 정면이 가까워져 좌우 바퀴 속도 차이로 부드럽게 회피 중 |
| `STATE PASS` | 좁은 통로의 측면 거리 때문에 속도를 낮추고 보정 중 |
| `STATE DANGER` | 매우 가까운 값이 잡혀 즉시 후진/회전 회피 |
| `STATE BLOCK` | 정면 중앙이 2번 연속 막혀 회피 회전 |
| `STATE STALL` | 전진 명령은 나갔지만 실제 바퀴 속도가 거의 0이라 복구 탐색 |
| `C숫자` | 중앙 영역에서 가장 가까운 거리(mm), 0은 유효값 없음 |
| `B숫자` | 가장 아래 행 `y=7`에서 가장 가까운 거리(mm) |
| `F숫자` | 최근 회피 실패 누적값. 커지면 부분 탐색/360도 탐색으로 넘어감 |
...
7. 부분 탐색이 랜덤 한계에 도달하면 360도 탐색 후 선택 방향 주변을 5도 단위로 정밀 탐색합니다.
...
function 현재방향세밀탐색(): number {
    상태 = "FINE"
    lcd표시(true)
    let 후보각 = [0, 0, 0, 0, 0]
    let 후보점수 = [-999999, -999999, -999999, -999999, -999999]
    후보삽입(후보각, 후보점수, 0, 정밀탐색점수())

    let 첫방향 = randint(0, 1) == 0 ? 1 : -1
    for (let i = 1; i <= 3; i++) {
        각도회전(첫방향, 세밀탐색간격도)
        후보삽입(후보각, 후보점수, 첫방향 * i * 세밀탐색간격도, 정밀탐색점수())
    }
    각도회전(0 - 첫방향, 세밀탐색간격도 * 3)

    let 둘째방향 = 0 - 첫방향
    for (let j = 1; j <= 3; j++) {
        각도회전(둘째방향, 세밀탐색간격도)
        후보삽입(후보각, 후보점수, 둘째방향 * j * 세밀탐색간격도, 정밀탐색점수())
    }
    각도회전(첫방향, 세밀탐색간격도 * 3)
...
| `정지거리mm` | 150 | 너무 가까이 붙으면 170~220으로 올림 |
...
| `통로측면거리mm` | 210 | 좁은 통로에서 벽에 닿지 않게 보정하기 시작하는 측면 거리 |
...
| `부분탐색한계` | 1~10 랜덤 | 이 횟수만큼 부분 탐색 후 360도+5도 정밀 탐색 |
