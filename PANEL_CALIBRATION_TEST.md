...
각도체크/패널테스트는 라이더가 물리적으로 똑바로 잡혀 있다는 전제로
동작하는 측정이라, 기울기가 안 맞은 상태로 먼저 돌리면 그 데이터 자체가
의미가 없어진다.
...
5. **1차 B 버튼**을 눌러 각도 raw 데이터 수집을 시작한다. 이 단계에서는
   ...
   것을 권장한다.
6. 전송이 끝나면(LCD에 "ANGLE DATA SENT") 패널을 다시 정위치에 놓고, **2차 B
   버튼**을 눌러 본 테스트를 시작한다.
...
function lcd대기표시(): void {
    ...
    lcd문자(5, 8, 168, "TOUCH = TILT CAL", 0xff8800)
}
...
- 라이더 높이는 물리적으로 140mm(`기본라이더높이mm`)에 맞춰 둔다 — 이
  ...
  동작이니 혼동하지 않는다.
- **기울기는 높이를 맞춘 다음, 다른 모드(B/AB)를 돌리기 전에 가장 먼저**
...
- [ ] 부팅 직후 5x5 매트릭스에 대기 아이콘, LCD에 "PANEL DATA TEST /
      HEIGHT 140mm / B = ANGLE DATA / ... / TOUCH = TILT CAL"이
...
- [ ] **1차 B 버튼**을 누르면 LCD에 "ANGLE DATA... / EMPTY FLOOR"가
      ...
      끊김 없이 찍히는가.
- [ ] 전송이 끝나면 LCD에 "ANGLE DATA SENT / B = START PANEL TEST / ... /
      TOUCH = TILT CAL"이 표시되고, 이때까지는 패널 테스트(카운트다운/
      후진)가 시작되지 않는가.
