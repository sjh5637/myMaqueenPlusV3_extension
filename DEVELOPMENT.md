...
## 3-1. 버전 태그 번호는 끊지 말고 순서대로만 올릴 것

과거 이 저장소의 태그 기록을 보면 `v0.0.1` ~ `v0.0.7`까지 올라가다가 갑자기
`v2.2.30`으로 점프합니다. 이는 `pxt.json`의 `version`을 DFRobot 원본 확장의
버전 체계(`2.2.x`)에 맞추려고 한 번 수동으로 크게 바꾼 것이며, 그 이후
(`v2.2.31` → `v2.2.36`)는 정상적으로 순서대로 올라갔습니다.

이미 push되어 외부에서 참조할 수 있는 태그(`v0.0.1`~`v2.2.36`)는 **다시 매기지
않습니다.** 태그를 재작성하려면 force push가 필요하고, 누군가 이미 특정 버전을
MakeCode 프로젝트에 추가해 사용 중이라면 그 참조가 깨질 수 있기 때문입니다.

앞으로는 **항상 직전 태그의 다음 patch 버전**으로만 올립니다 (예: 다음 릴리스는
`v2.2.37`). 마이너/메이저를 임의로 점프시키지 마세요. 새 태그를 만들기 전에
`git tag --sort=creatordate`로 현재 최신 태그를 확인하는 습관을 들이세요.
